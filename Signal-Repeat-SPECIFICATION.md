...
- Answer表示後に利用可能にする
- 評価ボタンの操作を妨げない
- Set/Listのネイティブな子項目表示を置換または抑制しない
- 起動ボタンはネイティブ回答領域の下に配置する
- Signal Repeat終了後は同じカード画面に戻る
- Signal Repeat自体はカード評価を変更しない
- 未検証のマルチラインカードは、親Remの`text`/`backText`から回答を推測しない
- `cardId`または文脈Remと一致する現在カードから方向を確定できる通常の順方向Setは、
  `isCardItem()`で特定した直接の子Remを元の順序で改行結合し、RichTextのまま表示する
- 編集中に残る構造上空のカード項目は回答から除外する。ただし非空の未対応RichText要素は
  黙って欠落させず、そのカードを非対応とする
- 同じ方法で方向を確定できる逆方向マルチラインカードは、直近の親Remの`text`を
  回答として表示する
- 番号付きのList、Partial、再帰展開、または公開SDKで現在カードを照合できない場合
  など、今回表示された回答を特定できない形式は引き続き非対応とする
...
- 残時間はプログレスバーで示す
- アニメーションは最小限とする
- プログレス表示は開始時に一度だけ設定するCSSアニメーションとし、進行中の
  DOM属性変更や対象RichText／メディアの再描画を発生させない
- RichTextの幅をスクロール領域へ強制せず、SDKの実測サイズと中央配置を安定させる
- テキスト選択やコピーを妨げない
- 長文でもスクロール可能にする
...
## AC-10 Supported multi-line card

Given:
公開SDKの正確なカード方向、文脈Rem、`isCardItem()`、`isListItem()`から、通常の
順方向Setまたは逆方向マルチラインカードの回答を正確に特定できる

When:
Signal Repeatを起動する

Then:
順方向Setでは直接の回答項目を元の順序で改行表示し、逆方向では直近の親Rem本文を
表示する

And:
各項目の書式、リンク、Rem参照、LaTeX、対応メディアを保持する
