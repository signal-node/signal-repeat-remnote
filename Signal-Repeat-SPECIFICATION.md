...
- `cardId`で方向を確定できる通常の順方向Setは、`isCardItem()`で特定した直接の
  子Remを元の順序で改行結合し、RichTextのまま表示する
- `cardId`で方向を確定できる逆方向マルチラインカードは、直近の親Remの`text`を
  回答として表示する
- 番号付きのList、Partial、再帰展開、`cardId`欠落など、公開SDKで今回表示された
  回答を特定できない形式は引き続き非対応とする
...
Given:
公開SDKの`cardId`、カード方向、`isCardItem()`、`isListItem()`から、通常の
