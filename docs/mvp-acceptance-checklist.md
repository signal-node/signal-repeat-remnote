...
| AC-09 Unsupported multi-line card | `remnoteAdapter.test.ts`, `targetResolver.test.ts`, and `startRepeat.test.ts` verify Powerup/card-item detection, fixed notification, no popup, and no focused-Rem fallback. | The original all-disabled gate passed with a synthetic Set on Desktop and Web. A 2026-09-13 Desktop real-queue check confirmed that a forward List shows the fixed unsupported notification and opens no popup. Partial, recursive, and unmatched-current-card variants remain disabled by tests rather than reconstructed. |
| AC-10 Supported multi-line card | `remnoteAdapter.test.ts` verifies ordered RichText joining for direct Set items, empty editor-placeholder omission, backward parent answers, contextual parent resolution when `Card.getRem()` omits multi-line metadata, exact current-card matching, and rejection of List, Partial-item, recursive, all-empty, and non-empty unsupported shapes. `flashcardAnswer.test.tsx` verifies both `cardId` and matched-current-card paths. | On 2026-09-13, Desktop real-queue checks passed for a synthetic forward Set (ordered answers displayed on separate lines) and an existing backward multi-line card (immediate parent displayed). Both used **Don't record answer choices**; no rating or schedule change occurred. Web regression remains pending because no authenticated Web session was available. |
