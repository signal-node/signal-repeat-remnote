...
| AC-09 Unsupported multi-line card | `remnoteAdapter.test.ts`, `targetResolver.test.ts`, and `startRepeat.test.ts` verify Powerup/card-item detection, fixed notification, no popup, and no focused-Rem fallback. | The original all-disabled gate passed with a synthetic Set on Desktop and Web. Current List/Partial/recursive and missing-card-ID variants remain disabled rather than reconstructed. |
| AC-10 Supported multi-line card | `remnoteAdapter.test.ts` verifies ordered RichText joining for direct Set items, backward parent answers, and rejection of List, Partial-item, and recursive shapes. `flashcardAnswer.test.tsx` verifies exact-card resolution without the ambiguous `remId` gate. | Pending Desktop/Web verification for a forward Set and backward multi-line card with a supplied `cardId`. |
